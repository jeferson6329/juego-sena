...
import {
  User, Star, CheckCircle2, Trophy,
  Zap, BookOpen, Target, Award,
} from 'lucide-react'

const BADGES = [
  { id: 'first_section',  emoji: '🌱', label: 'Primer paso',       desc: 'Completaste tu primera sección',  req: (done)           => done >= 1 },
  { id: 'guia1_complete', emoji: '🔵', label: 'Experto Back-end',  desc: 'Completaste toda la Guía 1',      req: (done, g1)       => g1.pct === 100 },
  ...
  { id: 'five_sections',  emoji: '🧠', label: 'Cuestionarista',    desc: 'Completaste 5 secciones',         req: (done)           => done >= 5 },
  { id: 'pts_100',        emoji: '⭐', label: '100 puntos',        desc: 'Acumulaste 100 puntos',           req: (done,g1,g3,pts) => pts >= 100 },
  { id: 'pts_300',        emoji: '💫', label: '300 puntos',        desc: 'Acumulaste 300 puntos',           req: (done,g1,g3,pts) => pts >= 300 },
...
function BadgeCard({ badge, unlocked }) {
  return (
    <div className={`flex flex-col items-center text-center p-4 rounded-xl border transition-all ${
      unlocked ? 'bg-sena-green/10 border-sena-green/30' : 'bg-gray-800/30 border-gray-800 opacity-40 grayscale'
    }`}>
...
export default function PerfilPage() {
  const { nombre, puntos, rol } = useAuth()
